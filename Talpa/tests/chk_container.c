...
int main(int argc, char *argv[])
{
    ...
    /*
     * file_path = dir + "/" + file + " (container hostname=" + hostname + ")"
     * e.g. "/test/file (container hostname=name)"
     */
    strcpy(file_path, dir);
    strcat(file_path, "/");
    strcat(file_path, file);
    strcat(file_path, " (container hostname=");
    ...
    if ( !rc )
    {
        ...
        ret = unshare(CLONE_NEWNS | CLONE_NEWUTS);
        if( ret < 0 )
        {
            fprintf(stderr, "unshare failed! %d %d\n", ret, errno);
            return -1;
        }
        ...
        if( ret < 0 )
        {
            fprintf(stderr, "sethostname failed! %d %d\n", ret, errno);
            return -2;
        }
